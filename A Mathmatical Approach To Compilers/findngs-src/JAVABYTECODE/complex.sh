...
input_size=$(stat -c%s Complex.java)
start_time=$(date +%s%3N)
compile_memory=$(/usr/bin/time -v javac Complex.java 2>&1 | grep "Maximum resident set size" | awk '{print $6}')
end_time=$(date +%s%3N)
compile_time=$((end_time - start_time))

output_binary="Complex.class"
...
rm -f Complex.java Complex.class
