...
compile_memory=$(/usr/bin/time -v clang -S -emit-llvm -o simple.ll simple.c 2>&1 | grep "Maximum resident set size" | awk '{print $6}')
clang simple.ll -o simple
end_time=$(date +%s%3N)
compile_time=$((end_time - start_time))
output_size=$(stat -c%s simple)
